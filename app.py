...
def extract_dates_v60(text):
    """
    v62.2 更新：加入 YYYY.MM.DD 格式支援 (CTI)
    """
    lines = text.split('\n')
    candidates = []
        ...
        for table in tables:
            ...
            for row in table:
                ...
                if re.search(r"(?i)(\bN\.?D\.?|\bNot Detected)", raw_res):
                    ...
                    if nums: final_val = nums[0] # 取第一個數字
...
st.set_page_config(page_title="SGS/CTI 報告聚合工具 v62.2", layout="wide")
st.title("📄 萬用型檢測報告聚合工具 (v62.2 CTI 專用分流版)")
st.info("💡 v62.2：針對 CTI 報告新增專用處理邏輯 (解決日期/鹵素總和/PFOS 關鍵字)，SGS 標準報告邏輯保持不變。")
...
if uploaded_files:
    ...
    try:
        ...
        st.download_button(
            ...
            file_name="SGS_CTI_Summary_v62.2.xlsx",
